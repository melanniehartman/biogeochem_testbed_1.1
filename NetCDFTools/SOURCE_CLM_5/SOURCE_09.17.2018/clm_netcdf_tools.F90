...
!     if (status /= nf_noerr) call handle_err(status, "LONGXY")
      ...
      !!status = nf_inq_varid(ncid, "PCT_NAT_PFT", pctpft_varid)
      if (status /= nf_noerr) call handle_err(status, "PCT_NAT_PFT")
...
!BOP
!
! !IROUTINE: readNbedrock
!
! !INTERFACE:
   subroutine readNbedrock(nchisth0filename, nbedrock)

! !DESCRIPTION:
!  Read nbedrock from a monthly histry file.
!
! !USES:
!  none
!
! !ARGUMENTS:
   character(len=*), intent(in) :: nchisth0filename     ! netcdf filename for monthly history file with nbedrock variable
...
! !LOCAL VARIABLES:
      ...
      integer :: lat, lon, levsoi               ! ntcdf dimension sizes
      integer :: nbedrock_varid                 ! netcdf variable id
      integer :: start2(2), count2(2)           ! start and count arrays for reading 2-D data from netcdf files

!-----------------------------------------------------------------------
      if (verbose .ge. 0) print *, "Reading nbedrock data from file ", trim(nchisth0filename), "..."
...
! Note, in NetCDF file the dimensions appear in the opposite order: lsmlat, lsmlon (2-D) 
      ...
      if (verbose .ge. 0) print *, "Done reading nbedrock data from file ", trim(nchisth0filename)

   end subroutine readNbedrock
...
!  !ARGUMENTS:
   ...
   character*200 :: nchisth0filename            ! netcdf monthly history file with nbedrock variable
   ...
   call readInitFile(filesin, filetype, ncfilename, ncsrfcfilename, ncndepfilename, &
                     filetransco2, nchisth0filename, h, &
...
!  real(4), allocatable :: soilliqday(:,:,:,:) ! soilliqday(lon, lat, nlevsoi, day) soil liquid water BY LAYER (kg/m2) (vegetated landunits only) 
   ...
   call readInitFile(filesin, filetype, ncfilename, ncsrfcfilename, ncndepfilename, &
                     filetransco2, nchisth0filename, h, iyear1, imonth1, iday1, iyear2, &
                     imonth2, iday2, time1, time2)
! WW added watsat,watfc  
   call readSurfaceData(ncsrfcfilename, lsmlon, lsmlat, lsmpft, nlevsoi, &
                        pctpft, pctsand, pctclay, orgdens, watsat, watfc)

!  Read a separate file to get index of shallowest bedrock layer (nbedrock). 
!  The nbedrock variable does not exist in the surface data set. -mdh 9/17/2018
   call readNbedrock(nchisth0filename, nbedrock)
...
! !INTERFACE:
   subroutine readInitFile(filesin, filetype, ncfilename, ncsrfcfilename, ncndepfilename, &
                           filetransCO2, nchisth0filename, hindex, &
                           iyear1, imonth1, iday1, iyear2, imonth2, iday2, time1, time2)
!
! !DESCRIPTION:
...
!  !ARGUMENTS:
   ...
   character(len=*), intent(out)   :: nchisth0filename   ! name of monthly history file with nbedrock variable to read
