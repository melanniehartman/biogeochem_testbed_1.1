...
#    Melannie Hartman
#    melannie@ucar.edu
#    May 7, 2018 - CPER soil (14 layers to depth of 210 cm)
#    June 4, 2018 - Harvard Forest soil (15 layers to depth of 270 cm)
#
#  Description:
...
workdir <<- "E:\\dev\\NCAR\\CASACLM\\POINT\\Harvard_Forest\\DayCent_UV\\"
setwd(workdir)
...
outputFileName = paste(workdir, "met_HARVF_1900_2006.csv", sep="")
...
# Standard DayCent soil thicknesses for Harvard Forest 16 soil layers (cm) (270 cm depth)
#   Added layer 16 (250cm) to make up the difference between deepest DayCent layer and deepest CASA layer.
#   The site may not have 16 soil layers. Extra layers in this list are OK.
#dcsoilthickness = c(2, 3, 5, 10, 10, 15, 15, 15, 15, 15, 15, 30, 30, 30, 250) #CPER
dcsoilthickness = c(2, 3, 5, 10, 10, 15, 15, 15, 15, 15, 15, 30, 30, 45, 45, 190) # Harvard Forest
###dcsoildepths = c(2, 5,10, 20, 30, 45, 60, 75, 90,105,120,150,180,225,270, 460)
...
# The DayCent soil profile (270 cm) does not go as deep as the CASA soil profile (460cm).
...
for (idy in 1:maxDays)
{
    if (psyn[idy,colInDOY] < 366)
    {
        ...
        # casa layer 1: (2*lyr1 + 0.2*lyr2)/2.2
        # casa layer 2: (2.8*lyr2 + 3*lyr3)/5.8
        # casa layer 3: (2*lyr3 + 10*lyr4 + 3.4*lyr5)/15.4
        # casa layer 4: (6.6*lyr5 + 15*lyr6 + 15*lyr7 + 4.3*lyr8)/40.9
        # casa layer 5: (10.7*lyr8 + 15*lyr9 + 15*lyr10 + 15*lyr11 + 30*lyr12 + 22.8*lyr13)/108.5
        # casa layer 6: (7.2*lyr13 + 45*lyr14 + 45*lyr15 + 190*lyr16)/287.2
        ...
        outputSummary[idy2,colOutTsoil1+5]    = (7.2*soiltavg[idy,13] + 45.0*soiltavg[idy,14] + 45.0*soiltavg[idy,15] + 190.0*soiltavg[idy,16]) / casasoilthickness[6]
        outputSummary[idy2,colOutVSWC1+5]     = (7.2*vswc[idy,13]     + 45.0*vswc[idy,14]     + 45.0*vswc[idy,15]     + 190.0*vswc[idy,16])     / casasoilthickness[6]
