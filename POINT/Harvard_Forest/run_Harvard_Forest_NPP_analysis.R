###############################################################################
#
# run_Harvard_Forest_NPP_analysis.R
#
...
#     for control runs.
#   * create plots of total SOC and microbial C for 4 soil NPP 
...
nppDeltas <- c("NPPx1.0", "NPPx0.5", "NPPx1.5", "NPPx2.0")
...
# Read POINT_casa_1_trans.csv for each NPP scenario
...
ReadCASAresults = function ()
{
  ...
  for (dP in 1:length(nppDeltas))
  {
    colCnt <- colCnt + 1
  
    casaFileName <- paste("POINT_casa_1_trans_", nppDeltas[dP], ".csv", sep="")
    ...
    { 
      ...
      nCols <<- length(nppDeltas) + 1 # first column is time, remaining columns are scenarios
      nYrs <<- as.integer(nTimes/365)
  ...
  plotTitle = "Harvard Forest: CASA Soil C Change with NPP perturbations"
  units = "gC/m2"
  colCnt = 2
  ...
  for (dP in 2:length(nppDeltas))
  {
    colCnt = colCnt + 1
    lines(somscCASAData[,1],somscCASAData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dP]) 
  }
  legend(x="bottomleft",y="bottom",lwd=2,col=colvec,lty=1,legend=nppDeltas,ncol=1)
  
} # End function ReadCASAresults()

#-------------------------------------------------------------------------------------------------

# Read POINT_mimics_1_trans.csv files for each NPP perturbation
...
ReadMIMICSresults = function ()
{
  ...
  for (dP in 1:length(nppDeltas))
  {
    ...
    mimicsFileName <- paste("POINT_mimics_1_trans_", nppDeltas[dP], ".csv", sep="")
    ...
    { 
      ...
      nCols <<- length(nppDeltas) + 1  # first column is time, next columns are scenarios
      nYrs <<- as.integer(nTimes/365)
  ...
  endYr <- 2006
  colvec <<- c("blue","darkgreen","green","brown")
  plotTitle = "Harvard Forest: MIMICS Soil C Change with NPP perturbations"
  units = "gC/m2"
  colCnt = 2
  ...
  {
    colCnt = colCnt + 1
    lines(somscMIMICSData[,1],somscMIMICSData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dP]) 
  }
  legend(x="bottomleft",y="bottom",lwd=2,col=colvec,lty=1,legend=nppDeltas,ncol=1)
...
# Read POINT_corpse_1_trans.csv files for each NPP perturbation
...
ReadCORPSEresults = function ()
{
  ...
  for (dP in 1:length(nppDeltas))
  {
    ...
    corpseFileName <- paste("POINT_corpse_1_trans_", nppDeltas[dP], ".csv", sep="")
    ...
    { 
        ...
        nCols <<- length(nppDeltas) + 1 # First column is time, the following columns are scenarios
        nYrs <<- as.integer(nTimes/365)
  ...
  endYr <- 2006
  colvec <<- c("blue","darkgreen","green","brown")
  plotTitle = "Harvard Forest: CORPSE Soil C Change with NPP perturbations"
  units = "gC/m2"
  colCnt = 2
  ...
  {
    colCnt = colCnt + 1
    lines(somscCORPSEData[,1],somscCORPSEData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dP]) 
  }
  legend(x="bottomleft",y="bottom",lwd=2,col=colvec,lty=1,legend=nppDeltas,ncol=1)
...
#-------------------------------------------------------------------------------------------------
# Plot MIMICS and CORPSE microbial biomass for all NPP perturbations
...
Plot_Microb = function()
{
  ...
  legend(x="top",y="bottom",lwd=2,col=colvec,lty=1,legend=nppDeltas,ncol=1)
...
# Read and Plot CASA, MIMICS, and CORPSE results for all NPP perturbations
ReadCASAresults()
ReadMIMICSresults()
ReadCORPSEresults()
