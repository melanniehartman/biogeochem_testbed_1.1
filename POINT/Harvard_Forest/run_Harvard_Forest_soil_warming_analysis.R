###############################################################################
#
# run_Harvard_Forest_soil_warming_analysis.R
#
...
# Description:
#   This R script will 
#   * plot DayCent results from the Harvard Forest simulations
#     for two treatments (no N additions and N additions).
#   * create plots to compare DayCent results to CASA, MIMICS, and CORPSE 
#     for control runs.
#   * create plots of total SOC and microbial C for 4 soil temperature 
#     perturbations for CASA, MIMICS, and CORPSE.
#   
###############################################################################
...
# List of perturbations.  The control should be the first in the list
# so that it can be compared to DayCent.
soilDeltas <- c("+0C", "+2C", "+4C", "+6C")
...
PlotDataSOM = function ()
{
    ...
    plotTitle = "Harvard Forest: DayCent Total Soil C and Individual Pools"
    units = "gC/m2"
    PlotLayout()
...
PlotForestC = function ()
{
    ...
    plotTitle = "Harvard Forest: DayCent Total Tree C and Individual Pools"
    units = "gC/m2"
    PlotLayout()
...
PlotForestNPP = function ()
{
  ...
  plotTitle = "Harvard Forest: DayCent Total Tree NPP by part"
  units = "gC/m2/yr"
  PlotLayout()
...
PlotForestGPP = function (period)
{
  ...
  xrange <- c(startYr,endYr)
  PlotLayout()
  plotTitle = "Harvard Forest: DayCent Forest GPP"
...
# Read POINT_casa_1_trans.csv for each soil warming scenario
# and extract NPP, Rh, soil carbon, litter, fT, and fW.
# 
...
ReadCASAresults = function ()
{
  ...
  for (dT in 1:length(soilDeltas))
  {
    colCnt <- colCnt + 1
  
    casaFileName <- paste("POINT_casa_1_trans", soilDeltas[dT], ".csv", sep="")
    ...
    { 
      ...
      nCols <<- length(soilDeltas) + 1 # first column is time, remaining columns are scenarios
      nYrs <<- as.integer(nTimes/365)
  ...
  plotTitle = "Harvard Forest: CASA Soil C Change with Soil Warming"
  units = "gC/m2"
  colCnt = 2
  baseVal = somscCASAData[1,colCnt]
  xrange <- c(startYr,endYr)
  yrange <- c(min(somscCASAData[,2:5]-baseVal),max(somscCASAData[,2:5]-baseVal))
  plot(somscCASAData[,1],somscCASAData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[1],lty=1,main=plotTitle,xlab="",ylab=units,xlim=xrange,ylim=yrange)
  for (dT in 2:length(soilDeltas))
  {
    colCnt = colCnt + 1
    lines(somscCASAData[,1],somscCASAData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dT]) 
  }
  legend(x="bottom",y="bottom",lwd=2,col=colvec,lty=1,legend=soilDeltas,ncol=1)
  
} # End function ReadCASAresults()

#-------------------------------------------------------------------------------------------------

# Read POINT_mimics_1_trans.csv files for each soil warming scenario
# and extract soil carbon and litter values
# 
...
ReadMIMICSresults = function ()
{
  ...
  for (dT in 1:length(soilDeltas))
  {
    ...
    mimicsFileName <- paste("POINT_mimics_1_trans", soilDeltas[dT], ".csv", sep="")
    ...
    { 
      ...
      nCols <<- length(soilDeltas) + 1  # first column is time, next columns are scenarios
      nYrs <<- as.integer(nTimes/365)
  ...
  endYr <- 2006
  colvec <<- c("blue","darkgreen","green","brown")
  plotTitle = "Harvard Forest: MIMICS Soil C Change with Soil Warming"
  units = "gC/m2"
  colCnt = 2
  baseVal = somscMIMICSData[1,colCnt]
  xrange <- c(startYr,endYr)
  yrange <- c(min(somscMIMICSData[,2:5]-baseVal),max(somscMIMICSData[,2:5]-baseVal))
  
  plot(somscMIMICSData[,1],somscMIMICSData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[1],lty=1,main=plotTitle,xlab="",ylab=units,xlim=xrange,ylim=yrange)
  for (dT in 2:length(soilDeltas))
  {
    colCnt = colCnt + 1
    lines(somscMIMICSData[,1],somscMIMICSData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dT]) 
  }
  legend(x="bottom",y="bottom",lwd=2,col=colvec,lty=1,legend=soilDeltas,ncol=1)
...
# Read POINT_corpse_1_trans.csv files for each soil warming scenario 
# and extract soil carbon and Rh values
# 
...
ReadCORPSEresults = function ()
{
  ...
  for (dT in 1:length(soilDeltas))
  {
    ...
    corpseFileName <- paste("POINT_corpse_1_trans", soilDeltas[dT], ".csv", sep="")
    ...
    { 
        ...
        nCols <<- length(soilDeltas) + 1 # First column is time, the following columns are scenarios
        nYrs <<- as.integer(nTimes/365)
  ...
  endYr <- 2006
  colvec <<- c("blue","darkgreen","green","brown")
  plotTitle = "Harvard Forest: CORPSE Soil C Change with Soil Warming"
  units = "gC/m2"
  colCnt = 2
  baseVal = somscCORPSEData[1,colCnt]
  xrange <- c(startYr,endYr)
  yrange <- c(min(somscCORPSEData[,2:5]-baseVal),max(somscCORPSEData[,2:5]-baseVal))
  plot(somscCORPSEData[,1],somscCORPSEData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[1],lty=1,main=plotTitle,xlab="",ylab=units,xlim=xrange,ylim=yrange)
  for (dT in 2:length(soilDeltas))
  {
    colCnt = colCnt + 1
    lines(somscCORPSEData[,1],somscCORPSEData[,colCnt]-baseVal,type="l",lwd=2,col=colvec[dT]) 
  }
  legend(x="bottom",y="bottom",lwd=2,col=colvec,lty=1,legend=soilDeltas,ncol=1)
...
Plot_DC_CASA_GPP_NPP = function (period)
{
  ...
  if (period == "daily") 
  {
    plotTitle = "Harvard Forest: Daily Forest GPP"
    ...
    plotTitle = "Harvard Forest: Daily Forest NPP"
    ...
    plotTitle = "Harvard Forest: Annual Forest GPP"
    ...
    plotTitle = "Harvard Forest: Annual Forest NPP"
...
Plot_DC_CASA_SOILC = function ()
{
  ...
  plotTitle = "Harvard Forest: Above+Below Ground Soil C and Individual Pools"
  units = "gC/m2"
  PlotLayout()
  ...
  obsLabels = c("BW-20cm", "PH-20cm", "BW-30cm", "PH-30cm")
  ...
  points(obsSOC[,2],obsSOC[,4], pch="2") # 20cm
  points(obsSOC[,2],obsSOC[,5], pch="3") # 30cm
  #Text labels are not showing up 
  #points(c(obsSOC[,2],obsSOC[,2]),c(obsSOC[,4],obsSOC[,5])) 
  #textxy(c(obsSOC[,2],obsSOC[,2]),c(obsSOC[,4],obsSOC[,5]), obsLabels, origin=c(startYr,0))
...
Plot_ALL_SOILC = function ()
{
  ...
  plotTitle = "Harvard Forest: Above+Below Ground Soil C"
  units = "gC/m2"
  PlotLayout4()
...
Plot_ALL_Rh = function ()
{
  ...
  plotTitle = "Harvard Forest: Heterotrophic Respiration"
  units = "gC/m2/day"
  PlotLayout4()
...
Plot_DC_CASA_MIMICS_LITC = function ()
{
  ...
  plotTitle = "Harvard Forest: Litter C Pools"
  units = "gC/m2"
  PlotLayout4()
  
  colCntDC <- 3     # This is column for DayCent N-addition    
  colCntCASA <- 2   # CASA control run
  colCntMIMICS <- 2 # MIMICS control run
...
Plot_DC_CASA_fT_fW = function ()
{
  ...
  plotTitle = "Harvard Forest: Soil temperature and moisture decomposition factors"
  units = "fraction"
  PlotLayout()
...
# Plot MIMICS and CORPSE microbial biomass for all soil temperature perturbations
...
Plot_Microb = function()
{
  ...
  yrange <- c(min(micrMIMICSData[,2:5])-1,max(micrMIMICSData[,2:5])+1)
  plotTitle = "Harvard Forest: microbial biomass"
  ...
  for (dP in 2:length(soilDeltas))
  {
    colCnt = colCnt + 1
    lines(micrMIMICSData[,1],micrMIMICSData[,colCnt],type="l",lwd=2,col=colvec[dP]) 
  }
  
  yrange <- c(min(mickMIMICSData[,2:5])-1,max(mickMIMICSData[,2:5])+1)
  plotTitle = "Harvard Forest: microbial biomass"
  ...
  yrange <- c(min(micCORPSEData[,2:5])-1,max(micCORPSEData[,2:5])+1)
  plotTitle = "Harvard Forest: microbial biomass"
  ...
  legend(x="top",y="bottom",lwd=2,col=colvec,lty=1,legend=soilDeltas,ncol=1)
...
# Read and plot CASA, MIMICS, and CORPSE results for all soil temperature perturbations
ReadCASAresults()
ReadMIMICSresults()
ReadCORPSEresults()
...
Plot_DC_CASA_SOILC()
Plot_DC_CASA_MIMICS_LITC() # Include MIMICS
...
# Plot MIMICS and CORPSE comparisons
Plot_Microb()
